...
function loadStatsPage(){
	...
	for(row = 2; row<table.rows.length ;row++){
		oppname = table.rows[row].cells[1].innerHTML;
		homeS = table.rows[row].cells[2].innerHTML;
		oppS = table.rows[row].cells[3].innerHTML;
		if(oppS>homeS)
		{
			table.rows[row].cells[4].innerHTML = oppname;
			losses = losses	+ 1;
		}
		else
		{
...
function loadPlayersPage(){
	...
	for(i = 0; i<players.length; i++)
	{
		...
		t = t + '<a class ="dropdown-item" href="#" onclick="switchPlayers('+i +')">'+players[i].name+"</a>";
...
function switchPlayers(playerNum){
	...
	pic.src = players[playerNum].img;
	pic.alt = players[playerNum].alt;
